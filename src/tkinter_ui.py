...
def sound(flie_name):
    ...
    pyg.mixer.music.load(flie_name)
...
def fire_misile(num):
    global misile,misile_x,misile_y,misile_fire,misile_hit_list,y,dy,x,dx,score
    misile_x[num] += 0
    if y <= misile_y[num]+m_size_y and y+dy >= misile_y[num]:    
        if x <= misile_x[num]+m_size_x and x+dx >= misile_x[num]:
            if not misile_hit_list[num]:
                misile_hit_list[num] = True
                sound("sound1.mp3")
...
def callback(num):
    global x,y,dx,dy,cnt,score,Endtime,canvas,End,Start
